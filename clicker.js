...
const defaultStats = { Juicings: 0, goons_per_click: 1 };
...
function updateUpgradeButtons() {
  ...
  upgrades.forEach((upgrade, index) => {
    html += `
      <button class="upgrade" id="${upgrade.name}" >
        ...
        <span class="upgrade-GPM">JPS: ${upgrade.gpm} </span>
  ...
  document.querySelectorAll('.upgrade').forEach((button, index) => {
    button.addEventListener('click', () => {
      if (stats.Juicings >= upgrades[index].price) {
        stats.Juicings -= upgrades[index].price;
...
setInterval(() => {
  addJuicings(global_gpm);
  updateCount();
}, 1000);

function updateCount() {
  countDisplay.innerText = `${Math.round(stats.Juicings)} `;
...
function isEnough() {
  upgrades.forEach((upgrade, index) => {
    ...
    if (stats.Juicings >= upgrade.price) {
...
function addJuicings(num) {
  stats.Juicings = Math.round((stats.Juicings + num) * 10) / 10;
}

function click_eggplant() {
  addJuicings(stats.goons_per_click);
  updateCount();
  eggplant.cumming();
}
